...
class InferDataset(data.Dataset):
    def __init__(self, dataset_folder, queryset_folder):
        """Dataset with images from database and queries, used for validation and test.
        Parameters
        ----------
        dataset_folder : str, should contain the path to the val or test set,
        ...
        super().__init__()
        self.dataset_folder = dataset_folder
        self.queryset_folder = queryset_folder
        if not os.path.exists(self.dataset_folder):
            raise FileNotFoundError(f"Folder {self.dataset_folder} does not exist")
        if not os.path.exists(self.queryset_folder):
            raise FileNotFoundError(f"Folder {self.queryset_folder} does not exist")
